...
// $Id: main.php,v 1.35 2008/10/19 14:25:11 nobu Exp $
...
switch ($op) {
    case 'catsave':
        ...
        $vals  = [];
        foreach (['catname', 'catdesc', 'catpri', 'weight', 'catimg'] as $k) {
        ...
        if ($catid) {
            ...
            $xoopsDB->query('UPDATE ' . CATBL . ' SET ' . implode(',', $vals) . " WHERE catid=$catid");
        } else {
            $xoopsDB->query('INSERT INTO ' . CATBL . '(' . implode(',', array_keys($vals)) . ') VALUES (' . implode(',', $vals) . ')');
        }
    ...
    case 'delnotify':
        ...
        $result = $xoopsDB->queryF($sql = 'DELETE FROM ' . RVTBL . ' WHERE eid=0 AND rvid IN (' . implode(',', $dels) . ')');
    ...
    case 'catdel':
        ...
        $res = $xoopsDB->query('DELETE FROM ' . CATBL . ' WHERE catid IN (' . implode(',', $dels) . ')');
        redirect_header('?op=category', 1, _AM_DBUPDATED);
        exit;
    ...
    case 'impsave':
        ...
        if (!$res || 1 != $xoopsDB->getRowsNum($res)) {
        ...
        while ($data = $xoopsDB->fetchArray($res)) {
            if (!$xoopsDB->query('INSERT INTO ' . CATBL . ' VALUES (' . implode(',', array_map('dbquote', $data)) . ')')) {
                die('DB ERROR');
            }
        }
        redirect_header('?op=category', 1, _AM_DBUPDATED);
...
switch ($op) {
    case 'events':
        ...
        $result = $xoopsDB->query('SELECT o.*,edate,title,uid,status FROM ' . EGTBL . ' e LEFT JOIN ' . OPTBL . ' o ON e.eid=o.eid ORDER BY e.eid DESC', $max, $start);
        $n      = 0;
        ...
        while ($data = $xoopsDB->fetchArray($result)) {
            ...
            $edit = "<a href='../admin.php?eid=$eid' class='edit'>" . _EDIT . '</a>' . " <a href='?op=edit&eid=$eid' class='status'>" . _AM_EDIT . '</a>' . " <a href='../admin.php?op=delete&eid=$eid' class='delete'>" . _DELETE . '</a>';
    ...
    case 'notifies':
        ...
        if ($nc) {
            ...
            while ($data = $xoopsDB->fetchArray($result)) {
                ...
                echo "<tr class='$bg'><td align='center'><input type='checkbox' name='rm[]' value='$rvid' ></td>" . "<td>$date</td><td>$email $uinfo</td></tr>\n";
            }
    ...
    case 'edit':
        ...
        $result = $xoopsDB->query('SELECT u.uid,groupid,uname' . ' FROM ' . $xoopsDB->prefix('groups_users_link') . ' l, ' . $xoopsDB->prefix('users') . ' u WHERE l.uid=u.uid AND ' . '(groupid=1 OR groupid=' . $xoopsModuleConfig['group'] . ') GROUP BY u.uid ORDER BY uname');
    ...
    case 'summary':
        ...
        $result  = $xoopsDB->query('SELECT e.eid,if(x.exid,x.exid,0) exid, IF(exdate,exdate,edate) exdate,title,uid,status,persons,IF(x.reserved,x.reserved,o.reserved) reserved FROM '
                                   . EGTBL
                                   . ' e LEFT JOIN '
                                   . OPTBL
                                   . ' o ON e.eid=o.eid LEFT JOIN '
                                   . EXTBL
                                   . ' x ON e.eid=eidref ORDER BY exdate DESC,e.eid DESC', $max, $start);

        if ($count > $max) {
            echo $nav->renderNav();
        }
        echo "<ul class=\"toptab\"><li class=\"save\"><a href='main.php?op=summary_csv'>" . _MD_CSV_OUT . "</a></li></ul>\n";
        ...
        while ($data = $xoopsDB->fetchArray($result)) {
            ...
            $title    = "<a href='$show?$param'>" . $myts->htmlSpecialChars($data['title']) . '</a>';
            $uname    = XoopsUserUtility::getUnameFromId($data['uid']);
...
function show_categories()
{
    ...
    // display entries
    $showlist = [
        'catname' => _AM_CAT_NAME,
        'weight'  => _AM_WEIGHT,
        'catimg'  => _AM_CAT_IMG,
        'catdesc' => _AM_CAT_DESC,
        'count'   => _AM_COUNT,
        'op'      => _AM_OPERATION
    ];
    ...
    <ul class=\"toptab\">
    <li class=\"addDir\">
    <a href='main.php?op=category&catid=0'>" . _AM_CATEGORY_NEW . "</a>
    </li>
    <li class=\"import\">
    <a href='main.php?op=catimp'>" . _AM_CATEGORY_IMPORT . "</a>
    </li>
    </ul>\n";

    echo "<p class='evnavi'>" . _AM_COUNT . ' ' . $xoopsDB->getRowsNum($res) . '</p>';

    echo "<form action='main.php?op=catdel' method='post'>\n";
    echo "<table class=\"outer\">\n";
    echo '<tr><th>' . _DELETE . '</th><th>' . implode('</th><th>', $showlist) . "</th></tr>\n";
    $ndel = $n = 0;
    while ($data = $xoopsDB->fetchArray($res)) {
        ...
        if (!empty($img)) {
            $img = "<img src='$img' alt='$name' width='32' >";
        }
        $data['catimg'] = $img;
        $data['op']     = "<a href='main.php?op=category&catid=$id'>" . _EDIT . '</a>';
        if ($haschild) {
            $del = '-';
        } else {
...
function summary_csv()
{
    ...
    $result = $xoopsDB->query('SELECT e.eid,if(x.exid,x.exid,0) exid, IF(exdate,exdate,edate) exdate,title,uid,status,persons,IF(x.reserved,x.reserved,o.reserved) reserved FROM '
                              ...
                              . ' o ON e.eid=o.eid LEFT JOIN '
                              . EXTBL
                              . ' x ON e.eid=eidref ORDER BY exdate DESC,e.eid DESC');
    $out    = '"' . implode('","', ['ID', '', _MD_EXTENT_DATE, _AM_TITLE, _AM_POSTER, _MD_RESERV_PERSONS, _AM_RESERVATION]) . "\"\n";
    while ($data = $xoopsDB->fetchArray($result)) {
        $date   = eventdate($data['exdate']);
